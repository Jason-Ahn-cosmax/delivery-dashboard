...
1. Apps Script 편집기 상단의 **함수 선택** 드롭다운에서 `generateUploadToken` 선택
2. **▶ 실행** 클릭
...
5. **이 토큰 문자열을 메모해두세요.** 나중에 엑셀 업로드할 때 사용합니다.
   - 잊으면 `showUploadToken` 함수를 실행해서 다시 볼 수 있음
   - 유출 시 `generateUploadToken`을 다시 실행해 새 토큰 발급
...
| 권한자 변경 시 토큰 회수 | Apps Script에서 `generateUploadToken` 다시 실행 → 새 토큰 발급 → 새 권한자에게 전달 (기존 권한자의 브라우저에 저장된 토큰은 무효화됨) |
